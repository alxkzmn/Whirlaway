...
impl<AB: AirBuilderWithPublicValues> Air<AB> for KeccakSpongeAir {
    fn eval(&self, builder: &mut AB) {
        ...
        builder
            ...
            .assert_zero(local_seen_end.clone());
        builder
            .when(first_row_sel.clone())
            .assert_one(local_active.clone());
        ...
        // and check it against block_bits[i] for rate bits, or 0 for capacity bits.
        {
            ...
            for (bit_idx, local_block_bit) in local_block_bits.iter().enumerate().take(STATE_BITS) {
                let lane = bit_idx / 64;
                let z = bit_idx % 64;
                let x = lane % 5;
                let y = lane / 5;
        ...
        // Enforce block bits are boolean at the start of each permutation (step_flags[0] == 1).
        {
            ...
            let local_block_bits = &local_row[BLOCK_BITS_START..BLOCK_BITS_START + RATE_BITS];
            for bit in local_block_bits.iter().take(RATE_BITS) {
                let b: AB::Expr = bit.clone().into();
                builder
                    .when(gate.clone())
