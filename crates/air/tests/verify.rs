...
#[test]
fn test_air_verify_invalid_selectors() {
    let air = keccak_air::KeccakAir {};
    let mut trace = air.generate_trace_rows::<F>(1, 0);
    ...
    // Corrupt selectors so they violate local selector constraints.
    {
        let row = 0;
        let slice = &mut trace.values[row * width..(row + 1) * width];
        let cols: &mut KeccakCols<F> = slice.borrow_mut();
        cols.first_row_sel = F::TWO;
    }
    if height > 2 {
        let row = 1;
        let slice = &mut trace.values[row * width..(row + 1) * width];
        let cols: &mut KeccakCols<F> = slice.borrow_mut();
        cols.transition_sel = F::ZERO;
    }
