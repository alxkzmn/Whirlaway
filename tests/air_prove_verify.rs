...
fn test_proof_size_reasonable() {
    ...
    let proof_data = prover_state.proof_data();
    let proof_size = proof_data.len();

    // Proof should not be empty and should be reasonable size
    // (exact size depends on implementation, but should be > 0 and < some reasonable upper bound)
    assert!(proof_size > 0);
    assert!(proof_size < 10_000_000); // Keep a generous upper bound for CI stability.
}
