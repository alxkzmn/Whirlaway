# Whirlaway EVM Proof Serialization v2

This document defines the canonical wire format emitted by `evm_vectors` for on-chain verification.

## Goals

- Include all verifier-required proof material (Whirlaway AIR + WHIR + transcript data + public inputs).
...
## JSON schema (`p3-whirlaway-evm-proof-v2`)

```json
{
  "schema": "p3-whirlaway-evm-proof-v2",
...
- `extension_field` (`BinomialExtensionField<_,8>`): 8 base limbs in order, each 4-byte big-endian.
...
Whirlaway verifier integration MUST reject proofs where transcript `proof_data` is not fully consumed.
